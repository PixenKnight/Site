...
const mixedInputLabelVariants = {
	neutral: {
		backgroundImage: "linear-gradient(to right, #d1d5dc, #d1d5dc, #d1d5dc)",
	},
	mixed: {
		backgroundImage: "linear-gradient(to right, #fb2c36, #fd9a00, #fd9a00)",
	},
	error: {
		backgroundImage: "linear-gradient(to right, #fb2c36, #fb2c36, #fb2c36)",
	},
	warning: {
		backgroundImage: "linear-gradient(to right, #fd9a00, #fd9a00, #fd9a00)",
	},
};
...
function RouteComponent() {
	...
	useEffect(() => {
		if (nameTouched) setNameValid(name.length > 0);
	}, [name, nameTouched]);

	useEffect(() => {
		const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
		if (emailTouched) setEmailValid(emailRegex.test(email) && email.length > 0);
	}, [email, emailTouched]);

	useEffect(() => {
		if (msgTouched) setMsgValid(msg.length > 0);
	}, [msg, msgTouched]);
	...
	useEffect(() => {
		...
		if (phone.length === 0) {
			...
			if (phoneTouched) setPhoneValid(phoneRegex.test(phone));
		}
	}, [phone, phoneTouched]);
		...
		<div className="min-h-[calc(100vh-72px-68px)] bg-gray-950 via-slate-800 to-slate-900 pb-6">
			<section className="max-w-4xl mx-auto h-min p-8 text-gray-200 ">
				<div className="relative text-center items-center flex flex-col gap-6">
					...
					<p>
						If you'd like to get in touch, please leave your contact information
						below! I'll reach out as soon as possible. Whether it's for
						professional inquiries, collaborations, or just to say hello, I'm
						excited to hear from you!
					</p>
					<p>
						Form submissions are handled by the good folks at{" "}
						<a
			...
			<section className="max-w-4xl md:mx-auto mx-4 p-8 bg-gray-800 rounded-lg shadow-lg relative z-10">
				...
				>
					...
					<div className="flex flex-col">
						...
						<div className="flex space-x-4 not-md:gap-5 not-md:flex-col">
							<input
								...
								title={
									"Please enter your email address. This field is required and must be a valid email address."
								}
								value={email}
							...
							<input
								...
								title={
									"Please enter your phone number. Due to the amount of possibilities for valid numbers, I'm only warning you that this phone MAY not be correct. If you're sure this number is correct, please proceed."
								}
								value={phone}
					...
					<input
						...
						name="_autoresponse"
						value="Thank you for submitting your contact information! I'll get back to you as soon as possible. In the meantime, here's a copy of your submission."
					/>
					<input
						type="hidden"
						className="appearance-none"
					...
					<input
						type="text"
						className="appearance-none h-0 absolute"
						name="_honey"
					/>
				</form>
			</section>
