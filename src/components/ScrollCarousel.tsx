		...
		>
			<Photo
				...
				props={{
					imgProps: {
						animate: {
							outlineColor: props.selected === props.index ? "rgba(0, 184, 219, 1)" : "rgba(0, 184, 219, 0)"
						},
						initial: {
...
export default function ScrollCarousel(props: { photos: string[], altTexts: string[] }) {
	...
	useEffect (() => {
		...
		ul.addEventListener("wheel", (e) => {
			...
			setSelected(prev => Math.max(0, Math.min(prev + delta, props.photos.length - 1)))
		}, false)
	...
	return (
		<div className="relative w-full max-w-full flex flex-col">
			...
			<div className="flex flex-row justify-between items-center">
				...
				<Dots count={props.photos.length} selected={selected} setSelected={setSelected} setUserScrolling={setUserDiscreteScrolling}/>
			</div>
		</div>
	)
}